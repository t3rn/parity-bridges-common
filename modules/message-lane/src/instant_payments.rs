...
use bp_runtime::{NO_INSTANCE_ID, MESSAGE_LANE_MODULE_PREFIX, bridge_account_id};
use codec::Decode;
...
impl<AccountId, Currency> MessageDeliveryAndDispatchPayment<AccountId, AccountId, Currency::Balance>
	...
	AccountId: Debug + Default + Decode,
{
	...
	fn pay_delivery_and_dispatch_fee(submitter: &AccountId, fee: &Currency::Balance) -> Result<(), Self::Error> {
		Currency::transfer(submitter, &relayers_fund_account(), *fee, ExistenceRequirement::AllowDeath)
			.map_err(Into::into)
	}

	fn pay_relayer_reward(relayer: &AccountId, reward: &Currency::Balance) {
		let pay_result = Currency::transfer(&relayers_fund_account(), relayer, *reward, ExistenceRequirement::AllowDeath);
