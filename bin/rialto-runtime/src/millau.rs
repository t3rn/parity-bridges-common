...
use bp_message_lane::{
	...
	InboundLaneData, LaneId, Message,
};
...
pub type FromMillauMessageCallOrigin = pallet_bridge_call_dispatch::CallOrigin<
	bp_millau::AccountSigner,
	bp_rialto::AccountSigner,
	bp_rialto::Signature,
>;
...
pub struct FromMillauMessagePayload(pallet_bridge_call_dispatch::MessagePayload<
	bp_millau::AccountSigner,
	bp_rialto::AccountSigner,
	bp_rialto::Signature,
	crate::Call,
...
impl LaneMessageVerifier<bp_rialto::AccountId, ToMillauMessagePayload, bp_rialto::Balance> for Millau {
	...
	fn verify_message(
		...
		let millau_minimal_fee = millau_delivery_base_fee
			+ millau_dispatch_fee
			+ millau_confirmation_fee;
...
impl MessageDispatch<bp_millau::Balance> for FromMillauMessageDispatch {
	...
	fn dispatch_weight(message: &DispatchMessage<Self::DispatchPayload, bp_millau::Balance>) -> Weight {
		message.data.payload.as_ref().map(|payload| payload.0.weight).unwrap_or(0)
	}

	fn dispatch(message: DispatchMessage<Self::DispatchPayload, bp_millau::Balance>) {
		match message.data.payload {
			Ok(payload) => pallet_bridge_call_dispatch::Module::<crate::Runtime>::dispatch(
				INSTANCE,
